...
(function() {
    ...
    var e = document.getElementById('sjekksvar');
...
function checkAll() {
  var allChecked = document.getElementById("item1").checked &&
    document.getElementById("item2").checked &&
    document.getElementById("item3").checked &&
    document.getElementById("item4").checked;

  var nesteButton = document.getElementById("next");
  if (allChecked) {
    nesteButton.classList.remove("disabled");
    nesteButton.disabled = false;
  } else {
    nesteButton.classList.add("disabled");
    nesteButton.disabled = true;
  }
}

document.addEventListener("DOMContentLoaded", checkAll);
document.getElementById("myForm").addEventListener("change", checkAll);
